...
class SalaryUI:
    def display(self, df_builder):
        ...
        new_pension_contribution_percent = st.sidebar.slider("Your Pension Contribution (%)", min_value=0,
                                                             max_value=100,
        ...
        if st.sidebar.button("Add Salary"):
            ...
            logging.info(f"Added new salary entry: {st.session_state.salary_entries[-1]}")
        ...
        for i, entry in enumerate(st.session_state.salary_entries):
            with st.sidebar.expander(
                    f"Salary Entry {i + 1}: £{entry['gross_income']} for {entry['num_months']} months"):
                ...
                updated_company_match_percent = st.slider(f"Company Pension Match (%) for Entry {i + 1}",
                                                          min_value=0, max_value=100,
                                                          value=entry['company_match_percent'],
                                                          key=f"company_{i}")
                ...
                if st.button(f"Save Changes for Entry {i + 1}"):
                    ...
                    logging.info(f"Updated salary entry {i + 1}: {st.session_state.salary_entries[i]}")
                    st.session_state.df = df_builder.rebuild_dataframe()
                    st.success(f"Updated data for Entry {i + 1}.")

                if st.button(f"Delete Salary Entry {i + 1}"):
                    logging.info(f"Deleted salary entry {i + 1}: {st.session_state.salary_entries[i]}")
...
class ExpensesUI:
    def display(self, df_builder):
        ...
        if st.sidebar.button("Add Expense"):
            ...
            logging.info(f"Added new expense entry: {st.session_state.expense_entries[-1]}")
            st.session_state.df = df_builder.rebuild_dataframe()
        ...
        for i, entry in enumerate(st.session_state.expense_entries):
            with st.sidebar.expander(
                ...
                if st.button(f"Save Changes for Expense Entry {i + 1}"):
                    ...
                    logging.info(f"Updated expense entry {i + 1}: {st.session_state.expense_entries[i]}")
                    st.session_state.df = df_builder.rebuild_dataframe()
                    st.success(f"Updated data for Expense Entry {i + 1}.")

                if st.button(f"Delete Expense Entry {i + 1}"):
                    logging.info(f"Deleted expense entry {i + 1}: {st.session_state.expense_entries[i]}")
...
class HousingUI:
    def display(self, df_builder):
        ...
        new_appreciation_rate = st.sidebar.number_input("Yearly Property Appreciation (%)", min_value=0.0,
                                                        max_value=20.0,
        ...
        if mortgage:
            ...
            mortgage_term = st.sidebar.number_input("Mortgage Term (Years)", min_value=1, max_value=40, value=25,
                                                    key="mortgage_term")
            standard_rate = st.sidebar.number_input("Standard Variable Rate (%)", min_value=0.0, max_value=10.0,
                                                    value=4.0,
        ...
        if st.sidebar.button("Add House"):
            ...
            logging.info(f"Added new housing entry: {st.session_state.housing_entries[-1]}")
        ...
        for i, entry in enumerate(sorted(st.session_state.housing_entries, key=lambda x: x["month_acquisition"])):
            with st.sidebar.expander(f"{entry['house_name']} (Acquired Month {entry['month_acquisition']})"):
                ...
                updated_month_acquisition = st.number_input("Month of Acquisition", min_value=1,
                                                            value=entry['month_acquisition'],
                                                            key=f"month_acquisition_{i}")
                ...
                if updated_sale:
                    updated_month_sale = st.number_input("Month of Sale", min_value=updated_month_acquisition + 1,
                                                         value=entry["month_sale"] if entry["month_sale"] else updated_month_acquisition + 12,
                                                         key=f"month_sale_{i}")
                ...
                if st.button(f"Save Changes for {entry['house_name']}"):
                    ...
                    logging.info(f"Updated housing entry {i + 1}: {st.session_state.housing_entries[i]}")
                    st.session_state.df = df_builder.rebuild_dataframe()
                    st.success(f"Updated data for {updated_house_name}.")

                if st.button(f"Delete {entry['house_name']}"):
                    logging.info(f"Deleted housing entry {i + 1}: {st.session_state.housing_entries[i]}")
